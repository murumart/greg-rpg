...
func _physics_process(delta: float) -> void:
	...
	offset.x = SHAKE_MAX_OFFSET.x * shake_amount * randf_range(-1.0, 1.0) * OPT.screen_shake_intensity
	offset.y = SHAKE_MAX_OFFSET.y * shake_amount * randf_range(-1.0, 1.0) * OPT.screen_shake_intensity
