...
func sanimate(anim: StringName, speed := 1.0) -> void:
	if not anim:
...
func tanim_shake(amt: int, strength: float, speed := 1.0) -> void:
	var tw := animated_sprite.create_tween().set_trans(Tween.TRANS_CUBIC)
	for i in amt:
		var s := (1.0 - i / float(amt)) * strength
		tw.tween_property(animated_sprite, "position:x", -s, 0.01 / speed)
		tw.tween_property(animated_sprite, "position:x", s, 0.01 / speed)
	tw.tween_property(animated_sprite, "position:x", 0, 0.01 / speed)
	await tw.finished


func tanim_bounce(strength: float, speed := 1.0) -> void:
	var tw := animated_sprite.create_tween().set_trans(Tween.TRANS_CUBIC)
	tw.tween_property(animated_sprite, "scale", Vector2(1.0 / strength, strength), 0.025 / speed)
	tw.tween_property(animated_sprite, "scale", Vector2(strength, 1.0 / strength), 0.05 / speed)
	tw.tween_property(animated_sprite, "scale", Vector2.ONE, 0.05 / speed)
	await tw.finished
