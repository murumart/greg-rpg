...
func die() -> void:
	...
	if background:
		...
		tw.tween_interval(2)
	await tw.finished
	SOL.dialogue("vampire_battle_dead")
	await SOL.dialogue_closed
	super()
...
func _progress_check() -> void:
	if character.health / character.max_health < 0.7 and progress == 0:
		progress = 1
	...
	elif character.health / character.max_health < 0.5 and progress == 1:
		progress = 2
	...
	elif character.health / character.max_health < 0.3 and progress == 2:
		progress = 3
