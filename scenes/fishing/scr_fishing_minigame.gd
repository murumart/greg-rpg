...
func line_movement(delta : float) -> void:
	for pos in hook_positions:
		...
		pos.y -= (speed * delta * randf_range(0.8, 1.2)) if state == States.MOVE else 0
...
func _on_hook_collision(node: Node2D) -> void:
	...
	if node is TileMap or node.get_parent().get("hazardous"):
		...
		if node.moving and not node.decor:
			...
			time_left += (40 + (combo * 20)) * clampf(2000 / depth, 0.2, 1.6)
...
func process_tilemap() -> void:
	...
	for x in 12:
		...
		if randf() < depth_fish_increase_curve.sample(depth / 10_000.0): spawn_fish(tilemap.to_global(tilemap.map_to_local(cell)), true)
