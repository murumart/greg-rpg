...
func cs1() -> void:
	...
	dlg2.reset().set_char("silent").add_line(dlg.ml("show me you are capable"))
	dlg.reset().set_char("grandma_talk").add_line(dlg.ml("it's nice to gift flowers."))
	dlg2.speak(dialogue_box)
	await dlg.speak_choice()
	dialogue_box.close()

	dlg2.reset().set_char("silent").add_line(dlg.ml("of taking them on"))
	dlg.reset().set_char("grandma_talk").add_line(dlg.ml("maybe, if you find [color=%s]eight flowers..." % FLOWERCOLOR))
	dlg2.speak(dialogue_box)
	await dlg.speak_choice()
	dialogue_box.close()

	dlg2.reset().set_char("silent").add_line(dlg.ml("bring me their heads"))
	dlg.reset().set_char("grandma_talk").add_line(dlg.ml("assemble a nice bouquet."))
	dlg2.speak(dialogue_box)
	await dlg.speak_choice()
	dialogue_box.close()

	dlg2.reset().set_char("silent").add_line(dlg.ml("and i will crown yours"))
