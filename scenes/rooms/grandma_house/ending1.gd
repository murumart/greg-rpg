...
func play() -> void:
	...
	tw.parallel().tween_property(grandma, "modulate:a", 0.0, 1.5).set_ease(Tween.EASE_IN)
	await tw.finished
...
func _line_start(line: int) -> void:
	if line == 2:
		...
		grandma.random_movement_distance = 24
	elif line == 9:
...
DIALOGUE end_1
CHAR grandma
	okaaay... okaay... i get it...
INSTASKIP
	you're strong enough... you can
CHAR greg
	that's right ████!!! i demolished you right there
CHAR grandma
	...excuse me?
CHAR greg
	get outta my house
