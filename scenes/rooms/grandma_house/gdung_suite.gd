...
func add_generated_object(key: StringName, object: Dictionary, instance: Node2D) -> void:
	var rect := Rect2i(Vector2i(
			(instance.global_position - Vector2(8, 8)) / 16.0) - get_position(),
			object.get(GDUNGObjects.SIZE))
	generated_objects.append({"key": key, "object": object, "rect": rect, "instance": instance})
