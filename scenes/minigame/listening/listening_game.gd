...
static var CHANNELS := [
	...
	},
	{
		...
		K.CUTSCENE: (func() -> void:
			...
			await Math.timer(1.0)
			var dlg := DialogueBuilder.new()
			dlg.al("um... i would like to order...")
			dlg.al("a large pizz... with [color=f94]snails[/color]... and [color=49f]fish[/color]...")
			dlg.al("yes... deliver to [color=f99]beacon archipelago[/color]...")
			dlg.al("oh, my full name? it's [color=f99]president frankling[/color].")
			await dlg.speak_choice()
			SND.play_song("")
			r.queue_free()
			),
